...
namespace Yanitta
{
    ...
    public class ProcessMemory
    {
        ...
        public void Call64(IntPtr address, string src)
        {
            ...
            bytes.AddRange(BitConverter.GetBytes(0xDEAD));// ret val

            #region ASM

            // push eip (stored refernse to next inctruction)
            // sub rsp, 8
            bytes.AddRange(new byte[] { 0x48, 0x83, 0xEC, 0x08 });
            // mov dword [rsp+0], low32
            bytes.AddRange(new byte[] { 0xC7, 0x44, 0x24 });
            bytes.Add(0);
            ...
            bytes.Add(0x9C);
            // save registers
#warning add save registers

            // code
            // sub rsp, 0x38
            bytes.AddRange(new byte[] { 0x48, 0x83, 0xEC });
            bytes.Add(0x38);
            ...
            // xor r9d, r9d
            bytes.AddRange(new byte[] { 0x45, 0x33, 0xC9 });
            ...
            bytes.AddRange(new byte[] { 0x48, 0x83, 0xC4 });
            bytes.Add(0x38);
            ...
            bytes.Add(0x9D);
            //restore registers
#warning add restore registers
            // retn
            bytes.Add(0xC3);

            #endregion

            // inject only int address (0x7FFFFFFF)
